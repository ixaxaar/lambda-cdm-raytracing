...
__global__ void compute_forces_direct(const float4* __restrict__ positions,
...
__global__ void update_velocities(float3* __restrict__ velocities,
...
__global__ void update_positions(float4* __restrict__ positions,
...
__device__ inline float3 minimum_image(float3 dr, float box_size) {
    ...
    if (dr.x > half_box)
        dr.x -= box_size;
    else if (dr.x < -half_box)
        dr.x += box_size;

    if (dr.y > half_box)
        dr.y -= box_size;
    else if (dr.y < -half_box)
        dr.y += box_size;

    if (dr.z > half_box)
        dr.z -= box_size;
    else if (dr.z < -half_box)
        dr.z += box_size;
...
__global__ void compute_forces_tiled(const float4* __restrict__ positions,  // x,y,z,mass
                                     float3* __restrict__ forces,
                                     const int num_particles,
                                     const float box_size,
                                     const float softening2) {
    ...
    for (int tile = 0; tile < num_tiles; tile++) {
        ...
        if (idx < num_particles) {
            ...
            sh_positions[tid].x = sh_positions[tid].y = sh_positions[tid].z = sh_positions[tid].w =
                0.0f;
        }
        __syncthreads();
...
__global__ void compute_forces_all_pairs(const float4* __restrict__ positions,
                                         float3* __restrict__ forces,
                                         const int num_particles,
                                         const float box_size,
                                         const float softening2) {
...
__global__ void leapfrog_update(float4* __restrict__ positions,
...
__global__ void compute_energy(const float4* __restrict__ positions,
                               const float3* __restrict__ velocities,
    ...
    if (gid < num_particles) {
        ...
        // Kinetic energy
        ke = 0.5f * my_mass * (my_vel.x * my_vel.x + my_vel.y * my_vel.y + my_vel.z * my_vel.z);
...
void launch_force_computation(const float4* d_positions,
                              float3* d_forces,
                              int num_particles,
                              float box_size,
                              float softening,
...
void launch_leapfrog_update(float4* d_positions,
                            float3* d_velocities,
    ...
    leapfrog_update<<<grid_size, block_size, 0, stream>>>(d_positions,
                                                          d_velocities,
                                                          d_forces,
                                                          num_particles,
                                                          dt,
                                                          box_size,
                                                          scale_factor,
                                                          update_velocity_first);
}

void launch_energy_computation(const float4* d_positions,
    ...
    compute_energy<<<grid_size, block_size, shared_mem_size, stream>>>(d_positions,
                                                                       d_velocities,
                                                                       d_kinetic_energy,
                                                                       d_potential_energy,
                                                                       num_particles,
