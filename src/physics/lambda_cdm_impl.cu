...
__global__ void generate_initial_conditions(float4* positions,
                                            float3* velocities,
                                            curandState* states,
                                            int num_particles,
                                            float box_size,
...
class LambdaCDMSimulationImpl {
   public:
    ...
    LambdaCDMSimulationImpl(size_t num_particles, float box_size, const CosmologyParams& params)
        : params_(params),
          cosmology_(std::make_unique<CosmologyModel>(params)),
          ...
          d_kinetic_energy_(nullptr),
          d_potential_energy_(nullptr) {
        // Check CUDA device
    ...
    ~LambdaCDMSimulationImpl() {
        ...
        if (d_kinetic_energy_)
            cudaFree(d_kinetic_energy_);
        if (d_potential_energy_)
            cudaFree(d_potential_energy_);
    ...
    void initialize_particles() {
        ...
        generate_initial_conditions<<<grid_size, block_size>>>(d_positions_,
                                                               d_velocities_,
                                                               d_curand_states_,
                                                               num_particles_,
                                                               box_size_,
    ...
    void step(double dt) {
        ...
        // First half kick
        kernels::launch_leapfrog_update(d_positions_,
                                        d_velocities_,
                                        d_forces_,
                                        num_particles_,
                                        dt * 0.5,
    ...
    void compute_forces() {
        kernels::launch_force_computation(
            d_positions_, d_forces_, num_particles_, box_size_, softening_, streams_[0]);

        CUDA_CHECK(cudaStreamSynchronize(streams_[0]));
    }

    void compute_energy() {
        kernels::launch_energy_computation(d_positions_,
                                           d_velocities_,
                                           d_kinetic_energy_,
                                           d_potential_energy_,
                                           num_particles_,
        ...
        CUDA_CHECK(cudaMemcpy(
            &h_kinetic_energy_, d_kinetic_energy_, sizeof(float), cudaMemcpyDeviceToHost));
        CUDA_CHECK(cudaMemcpy(
            &h_potential_energy_, d_potential_energy_, sizeof(float), cudaMemcpyDeviceToHost));
...
LambdaCDMSimulation::LambdaCDMSimulation(size_t num_particles,
                                         float box_size,
