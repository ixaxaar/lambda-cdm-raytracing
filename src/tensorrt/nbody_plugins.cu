...
__global__ void nbody_force_kernel(const float* positions,
                                   const float* masses,
                                   float* forces,
                                   float* potential,
                                   int num_particles,
...
__global__ void nbody_force_kernel_shared(const float* positions,
                                          const float* masses,
                                          float* forces,
                                          float* potential,
                                          int num_particles,
    ...
    for (int tile = 0; tile < gridDim.x; ++tile) {
        ...
        if (i < num_particles) {
            for (int k = 0; k < blockDim.x; ++k) {
                ...
                if (global_j >= num_particles || global_j == i)
                    continue;

                float3 pos_j =
                    make_float3(shared_pos[k * 3], shared_pos[k * 3 + 1], shared_pos[k * 3 + 2]);
...
__global__ void tree_force_kernel(const float* positions,
                                  const float* masses,
                                  float* forces,
                                  int num_particles,
                                  float theta,
...
extern "C" {
void launch_nbody_force_kernel(const float* positions,
                               const float* masses,
                               float* forces,
                               float* potential,
                               int num_particles,
...
void launch_tree_force_kernel(const float* positions,
                              const float* masses,
                              float* forces,
                              int num_particles,
                              float theta,
...
nvinfer1::DimsExprs NBodyForcePlugin::getOutputDimensions(
    int outputIndex,
...
bool NBodyForcePlugin::supportsFormatCombination(int pos,
...
void NBodyForcePlugin::configurePlugin(const nvinfer1::DynamicPluginTensorDesc* in,
                                       int nbInputs,
...
size_t NBodyForcePlugin::getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs,
                                          int nbInputs,
...
int NBodyForcePlugin::enqueue(const nvinfer1::PluginTensorDesc* inputDesc,
    ...
    launch_nbody_force_kernel(
        positions, masses, forces, potential, num_particles, softening_length_, stream);

    return 0;
}

nvinfer1::DataType NBodyForcePlugin::getOutputDataType(int index,
...
nvinfer1::IPluginV2* NBodyForcePluginCreator::createPlugin(
    const char* name,
...
nvinfer1::DimsExprs TreeForcePlugin::getOutputDimensions(
    int outputIndex,
    const nvinfer1::DimsExprs* inputs,
    int nbInputs,
    nvinfer1::IExprBuilder& exprBuilder) noexcept {
...
bool TreeForcePlugin::supportsFormatCombination(int pos,
                                                const nvinfer1::PluginTensorDesc* inOut,
                                                int nbInputs,
                                                int nbOutputs) noexcept {
    return inOut[pos].type == nvinfer1::DataType::kFLOAT &&
...
void TreeForcePlugin::configurePlugin(const nvinfer1::DynamicPluginTensorDesc* in,
                                      int nbInputs,
                                      const nvinfer1::DynamicPluginTensorDesc* out,
                                      int nbOutputs) noexcept {}

size_t TreeForcePlugin::getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs,
                                         int nbInputs,
                                         const nvinfer1::PluginTensorDesc* outputs,
                                         int nbOutputs) const noexcept {
...
int TreeForcePlugin::enqueue(const nvinfer1::PluginTensorDesc* inputDesc,
    ...
    launch_tree_force_kernel(
        positions, masses, forces, num_particles, theta_, workspace, workspace_size, stream);

    return 0;
}

nvinfer1::DataType TreeForcePlugin::getOutputDataType(int index,
                                                      const nvinfer1::DataType* inputTypes,
                                                      int nbInputs) const noexcept {
    return nvinfer1::DataType::kFLOAT;
}
...
nvinfer1::IPluginV2* TreeForcePluginCreator::createPlugin(
    const char* name,
    const nvinfer1::PluginFieldCollection* fc) noexcept {
    float theta = 0.5f;
    int max_particles = 100000;
