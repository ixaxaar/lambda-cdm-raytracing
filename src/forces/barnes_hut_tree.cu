...
__global__ void compute_morton_codes_kernel(const float4* positions,
...
__global__ void build_internal_nodes_kernel(const uint32_t* sorted_morton_codes,
                                            TreeNode* nodes,
                                            int* node_count,
                                            int num_particles) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (tid > 0) {
        int delta_plus =
            common_prefix_length(sorted_morton_codes[tid], sorted_morton_codes[tid + 1]);
        int delta_minus =
            common_prefix_length(sorted_morton_codes[tid], sorted_morton_codes[tid - 1]);
    ...
    while (tid + l_max * d >= 0 && tid + l_max * d < num_particles &&
           common_prefix_length(sorted_morton_codes[tid], sorted_morton_codes[tid + l_max * d]) >
               delta_min) {
        l_max = l_max * 2;
    }
    ...
    for (int t = l_max / 2; t >= 1; t /= 2) {
        if (tid + (l + t) * d >= 0 && tid + (l + t) * d < num_particles &&
            common_prefix_length(sorted_morton_codes[tid], sorted_morton_codes[tid + (l + t) * d]) >
                delta_min) {
            l = l + t;
        }
    }
    ...
    for (int t = (l + 1) / 2; t >= 1; t = (t + 1) / 2) {
        if (tid + (s + t) * d < num_particles &&
            common_prefix_length(sorted_morton_codes[tid], sorted_morton_codes[tid + (s + t) * d]) >
                delta_node) {
            s = s + t;
        }
    }
...
__global__ void init_leaf_nodes_kernel(const float4* positions,
...
__global__ void compute_mass_centers_kernel(TreeNode* nodes, int* flags, int num_internal_nodes) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__device__ void traverse_tree(float4 particle_pos,
...
// Tree force kernel
__global__ void tree_force_kernel(const float4* positions,
                                  const TreeNode* nodes,
                                  float3* forces,
                                  int num_particles,
                                  float theta,
...
__global__ void tree_force_shared_kernel(const float4* positions,
                                         const TreeNode* nodes,
                                         float3* forces,
                                         int num_particles,
                                         float theta,
...
void BarnesHutTree::sort_particles() {
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_sorted_morton_codes_,
                               d_morton_codes_,
                               num_particles_ * sizeof(uint32_t),
                               cudaMemcpyDeviceToDevice,
                               build_stream_));
    CUDA_CHECK(cudaMemcpyAsync(d_sorted_indices_,
                               d_particle_indices_,
...
void BarnesHutTree::compute_forces(const float4* d_positions,
                                   float3* d_forces,
                                   float softening,
                                   cudaStream_t stream) {
